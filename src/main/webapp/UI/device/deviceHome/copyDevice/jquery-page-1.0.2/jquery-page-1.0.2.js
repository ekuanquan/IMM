...
;(function($,window,document,undefined){
    ...
    function initView(){
        ...
        $tipBox.text('0/1').attr('id',IDSet.tip);
        $jump.text('跳').attr('id',IDSet.jumpBtn);
        $label.text('页');
    ...
    _plugins.prototype.setPage = function(maxPage,currPage,total){
        ...
        var tipSTR = _format(defaults.tempelet,[currPage,maxPage]);
        ...
        if(maxPage <= 5){
            ...
            //当前页靠前
            if (currPage <= 3) {
                createTurn($turn,1,4,currPage,clickPage);
                var $i = createI(classSet.point);
                $turn.append($i);
                createTurn($turn,maxPage,maxPage,currPage,clickPage);
            }
            //当前页靠后
            else if (currPage >= maxPage - 2) {
                createTurn($turn,1,1,currPage,clickPage);
                var $i = createI(classSet.point);
                $turn.append($i);
                createTurn($turn,maxPage - 3,maxPage,currPage,clickPage);
            }
            //当前页在中间
            else{
                createTurn($turn,1,1,currPage,clickPage);
                ...
                $turn.append($i_1);
                createTurn($turn,currPage - 1,currPage + 1,currPage,clickPage);
