...
(function(window, $) {
	...
	function _exchangeText(ownerPojo) {
		/*
		 * 
		 * if (ownerPojo.userType == '0') { ownerPojo.userType = '一般客户'; } else
		 * if (ownerPojo.userType == '1') { ownerPojo.userType = '机主客户'; } else
		 * if (ownerPojo.userType == '2') { ownerPojo.userType = '操作员'; }
		 */
	...
	function _getSubmitOwnerInfoParams() {
		...
		// ownerPojo.createDate = _global.createDate;
		// ownerPojo.userId = $("#userId").val();
	...
	/*
	 * function _appendSelect() { for (var i = 0;i<dropDownPojo.length;i++){
	 * var $option = $("<option></option>");
	 * $option.attr('value',dropDownPojo[i].key);
	 * $option.text(dropDownPojo[i].value); $option.appendTo($("#centerName")); }
	 *  }
	 */
	...
	function validate_callback(data, field) {
		if (data.result.code == 0) {
			if (field == "userAccount") {
				$("#userId").next().removeClass("Validform_wrong").addClass(
						"Validform_right");
			}
			if (field == "userId") {
				$("#userId").next().removeClass("Validform_wrong").addClass(
			...
			if (field == "userAccount") {
				...
				$("#userId").next().removeClass("Validform_right").addClass(
						"Validform_wrong");
			}
			if (field == "userId") {
				// alert("该用户编号已经存在。");
