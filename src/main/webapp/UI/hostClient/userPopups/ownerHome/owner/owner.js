...
;(function (window, $) {
    ...
    function _initData() {
        ...
        if (parent.getTopPopupsName && typeof (parent.getTopPopupsName) == 'function') {
            ...
            console.log('如果getTopPopupsName不可用');
            //如果getTopPopupsName不可用，则判断是否有选中的数据
            if (parent.parent.getPopupsRowJson && typeof (parent.parent.getPopupsRowJson) == 'function') {
                var rowJson = parent.parent.getPopupsRowJson();
                if(rowJson.userId){
                    _global.popupsName = 'alterOwnerUser';
                }
            }
        }
        if (_global.popupsName == 'addOwnerUser') {
            //post_async(null,"/IntegratedMM/identifier/queryLatestId.do",_callback_getownerId)
            console.log('addOwnerUser');
            _addEvent();
            //默认录入时间为当前时间
            var nowTime = getNowFormatDate();
            //时间戳转换成 yyyy-MM-dd HH:mm:ss
            nowTime = formatDateTime(nowTime);
            //默认服务到期时间为当前时间
            $('#serveEndTime').val(nowTime);
            //时间戳转换成 yyyy-MM-dd
        ...
        $('#platformId').val(platformId.platform_name).data('key',platformId.platform_id);
    ...
    function _initEvent() {
        ...
        if (_global.popupsName == 'addOwnerUser') {
            ...
            $("#userId_sele").attr("maxlength","9").attr("datatype","16number").attr("nullmsg","用户编号不能为空").attr("errormsg","请输入用户编号").attr("onblur","isEmpty('userId_sele','2');");
            $("#userId_sele").bind("input propertychange",function(){
        ...
        //点击取消按钮，去除失焦事件
        var btn = document.querySelector('#cancelButton');
        ...
        $('#serveEndTime').click(function () {//服务到期时间
            WdatePicker({
    ...
    function _alterEvent() {
        ...
        $("#userId").bind("click",function() {
            _global.top.setUserIdO($("#userId").val());
            //_global.top._setIframeNameO();
            _global.top.openOtherPopus('../hostClient/userPopups/EditNumber/editNumber.html',{
                width: 400,
                height: 230
            });
        });
    ...
    function _showOwnerPojo(ownerPojo) {
       // ownerPojo = _exchangeText(ownerPojo);
        ...
        //$("#userAccount").append("<option>"+ownerPojo.userAccount+"</option>");
        $("#userPwd").val(ownerPojo.userPwd);
        $("#createDate").val(ownerPojo.createDate);
        $("#userId").val(ownerPojo.userId);
        //$("#userId").append("<option>"+ownerPojo.userId+"</option>");
        ...
        $('#platformId').val(ownerPojo.platformName).data('key',ownerPojo.platformId);
...
*/
    ...
    function _getSubmitOwnerInfoParams() {
        ...
        if (_global.popupsName == 'addOwnerUser') {
            ...
            //获取关联设备数据
            var alldev = parent.associatedDeviceIframe.setdevInfo();
            ownerPojo.devIds = alldev.devIds;           //关联设备
            ownerPojo.masterDevId = alldev.masterDevId; //主设备Id
            ownerPojo.remoteDevId = alldev.remoteDevId; //远程监控设备Id
        } else if (_global.popupsName == 'alterOwnerUser') {
       ...
       // ownerPojo.areaName = $("#areaName").val();
        ...
        ownerPojo.platformId = $('#platformId').data('key');
        ownerPojo.serveEndTime = $("#serveEndTime").val();  //服务到期时间
        ownerPojo.switchUser = $('#switchUser').val();//是否启用
    ...
    function callbackdelsuc() {
       ...
       if(params.ownerPojo.fMemo.length>=225){
    	   alert("备注过长")
    	   return;
       } 
        //alert(JSON.stringify(params));
        console.log("params: "+JSON.stringify(params));
        post_async(params, _config.ajaxUrl.alterOwnerUserInfoUrl, _callback_submitOwnerInfo,params);
    }

    function _callback_submitOwnerInfo(data,params) {
        var result = data.result;

        if (result.code == '0') {
            //alert("提交成功");
            if (_global.popupsName == 'alterOwnerUser') {
                _global.top.alertTip(result.message,0,null);
            }
            _global.top.RefreshhostClientIframe();
            _global.top.closePopus();
            if (_global.popupsName == 'addOwnerUser') {
                _global.top.userPopusManager('alterOwnerUser');
                _global.top.setPopupsRowJson(params.ownerPojo);
            }
    ...
    function validate_callback(data,field){
        if(data.result.code == 0){
            if(field == "userAccount"){
                $("#userAccount").next().removeClass("Validform_sigh").removeClass("Validform_wrong").addClass("Validform_right");
            }
            if(field == "userId_sele"){
                $("#userId_sele").next().removeClass("Validform_sigh").removeClass("Validform_wrong").addClass("Validform_right");
                _global.bool = true;
            }
            ...
            if(field == "userAccount"){
                ...
                $("#userAccount").next().removeClass("Validform_sigh").removeClass("Validform_right").addClass("Validform_wrong");
            }
            if(field == "userId_sele"){
                //alert("该用户编号已经存在。");
                _global.bool = false;
                _global.top.alertFail("该用户编号已经存在",2000,null);
                $("#userId_sele").next().removeClass("Validform_sigh").removeClass("Validform_right").addClass("Validform_wrong");
            }
    ...
    function _callback_getownerId(data) {
        ...
        $("#userId_editable-select-options ul li").bind("mousedown",function () {
