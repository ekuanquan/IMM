...
(function() {
    $.extend($.fn, {
        ...
        showBackground:function(setting,addJsonCallback){
            var _self = this;
        ...
        addiconList: function(optionsList) {
            ...
            var defaultsList = {
                ...
                delCallBack:null,
                setDraggableJson: null,
                mapId:null
            };
            var settingsList = $.extend(defaultsList, optionsList || {}),
                $this;
            $.fn.showBackground(settingsList,addJsonCallback);

            function addJsonCallback() {
                if (settingsList.dataList.length > 0) {
                    for (var i = 0; i < settingsList.dataList.length; i++) {
                        if ((settingsList.dataList[i].x == null || settingsList.dataList[i].x == 0 || settingsList.dataList[i].x == "") &&
                            ...
                            if (settingsList.mapId == mapId) {
                                ...
                                $.fn.initialize(settingsList, settingsList.dataList[i].ownerZoneName, x, y, i);
                                addJson(settingsList.iconId + i, settingsList.dataList[i].x, settingsList.dataList[i].y,
                                    settingsList.dataList[i].ownerZoneName, settingsList.wrapperId,settingsList.dataList[i].mapId);
                    ...
                    if (settingsList.setDraggableJson && typeof settingsList.setDraggableJson == 'function') {
                        settingsList.setDraggableJson();
                    }
                }
                ...
                (function () {
                    ...
                    var $box = $('#' + optionsList["wrapperId"]).mousedown(function (e) {
                        ...
                        this.posix = {
                            'x': e.pageX - offset.left + cOffset.left,
                            'y': e.pageY - offset.top + cOffset.top
                        };
                        $.extend(document.getElementById(optionsList["containerId"]), {
                            'move': true,
                            'move_target': this
                        });
                    })
            ...
            function addJson(id,x,y,number,divid,mapId) {//创建json
