...
(function(window, $, undefined) {
	...
	function _addRow(row_json) {
		...
		$div_row.append($cameraName).append($devChannelId)
				.append($devMonitorId).append($atPos).append($instDate).append(
						$almType).append($wantDo).append($cameraType).append(
						$cameraModeId).append($gbId).append($fMemo).addClass(
						'row');
		$cameraName.addClass('table_item_4').text(row_json.cameraName).attr(
				"title", row_json.cameraName);
		$devChannelId.addClass('table_item_4').text(row_json.devChannelId)
				.attr("title", row_json.devChannelId);
		$devMonitorId.addClass('table_item_4').text(row_json.devMonitorId)
				.attr("title", row_json.devMonitorId);
		$atPos.addClass('table_item_4').text(row_json.atPos).attr("title",
				row_json.atPos);
		$instDate.addClass('table_item_4').text(row_json.instDate).attr(
				"title", row_json.instDate);
		if (row_json.almTypeName == "" || row_json.almTypeName == null) {
			row_json.almTypeName = row_json.almType
		}
		;
		$almType.addClass('table_item_4').text(row_json.almTypeName).attr(
				"title", row_json.almTypeName);
		if (row_json.wantDoName == "" || row_json.wantDoName == null) {
			row_json.wantDoName = row_json.wantDo
		}
		;
		$wantDo.addClass('table_item_4').text(row_json.wantDoName).attr(
				"title", row_json.wantDoName);
		...
		;
		$cameraType.addClass('table_item_4').text(row_json.cameraTypeName)
				.attr("title", row_json.cameraTypeName);
		if (row_json.cameraModelName == "" || row_json.cameraModelName == null) {
			row_json.cameraModelName = row_json.cameraModelId
		}
		;
		$cameraModeId.addClass('table_item_4').text(row_json.cameraModelName)
				.attr("title", row_json.cameraModelName);
		$gbId.addClass('table_item_4').text(row_json.gbId).attr("title",
				row_json.gbId);
	...
	function getcameraType(cameraType) {
		...
		var cameraTypedata = post_sync({
			"cameraType" : cameraType
		}, "/IntegratedMM/queryCameraTypeById.do");
		var cameraTypeName = "";
