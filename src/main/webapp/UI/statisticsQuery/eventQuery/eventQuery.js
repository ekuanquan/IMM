...
//重绘布局
    ...
    function _searchEventInfo() {
        ...
        alertPojo.disposeStatus = $("#disposeStatus").val();
    ...
    function addTableRow(jsonData, isPre) {
        ...
        dataResult.result = parent.parent.getAlarmReason();
        for(var i=0;i<dataResult.result.length;i++){
            if(dataResult.result[i].alarmcode==row_json.actualSituation){
                result=dataResult.result[i].alarmcontent;
            }
        }
        $div_actualSituation .addClass("table_item_time").text(result).attr("title", result);
        ...
        $div_acceptAlarmer .addClass("table_item_4").text(row_json.acceptAlarmer).attr("title", row_json.acceptAlarmer);
    ...
    function _messageExchange(dataJson) {
        ...
        row_json.docResult = dataJson.docResult;
        var acceptAlarmer = dataJson.acceptAlarmer;
        if(acceptAlarmer ==null|| acceptAlarmer =="null"){
            acceptAlarmer ="";
        }
        row_json.acceptAlarmer = acceptAlarmer;
    ...
    function _getAlarmInfos(alertPojo) {
        ...
        var params = {
            ...
            "cphone": param.alertPojo.cMobile,
            "roleId": _global.top.getSysroleId(),
            "disposeStatus":param.alertPojo.disposeStatus,
            "pageInfoPojo": {
