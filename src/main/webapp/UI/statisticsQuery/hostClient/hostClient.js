...
;(function ($, window) {
    ...
    function _initEven() {
        ...
        /************************************************
         备注的升降序的切换222
         ************************************************/
        $("#userInfoTab tr th:eq(18)").click(function() {
            $("#businessPng").removeClass("businessPngchange").removeClass("businessPngchange2");
            _global.flag2 = 0;
            _global.flag += 1;
            var tmp = _global.flag % 3;
            console.log("tmp:" + tmp);
            if (tmp == 1) {
                $("#fMemoPng").removeClass("fMemoPngchange2").addClass("fMemoPngchange");
            }
            else if (tmp == 2) {
                $("#fMemoPng").removeClass("fMemoPngchange").addClass("fMemoPngchange2");
            } else if (tmp == 0) {
                $("#fMemoPng").removeClass("fMemoPngchange").removeClass("fMemoPngchange2");
            }
            /*var sort = "";
            if ($("#fMemoPng").hasClass("fMemoPngchange")) {
                sort = "fMemo|DESC";
            } else if ($("#fMemoPng").hasClass("fMemoPngchange2")) {
                sort = "fMemo|ASC";
            } else {
                sort = sortVal();
            }*/
            ...
            params.pageInfoPojo.sort = sort;
            post_async(params, _config.ajaxUrl.getCphUserDataUrl, _callback_getAlarmInfos);
        });
        /************************************************
         子行业的升降序的切换222
         ************************************************/
        $("#userInfoTab tr th:eq(15)").click(function() {
            $("#fMemoPng").removeClass("fMemoPngchange").removeClass("fMemoPngchange2");
            _global.flag = 0;
            _global.flag2 += 1;
            var tmp = _global.flag2 % 3;
            console.log("tmp:" + tmp);
            if (tmp == 1) {
                $("#businessPng").removeClass("businessPngchange2").addClass("businessPngchange");
            }
            else if (tmp == 2) {
                $("#businessPng").removeClass("businessPngchange").addClass("businessPngchange2");
            } else if (tmp == 0) {
                $("#businessPng").removeClass("businessPngchange").removeClass("businessPngchange2");
            }
            /*var sort = "";
    ...
    function sortVal() {
        ...
        var businesssort = "";
        if($("#businessPng").hasClass('businessPngchange')){
            businesssort="define1|DESC";
        }
        else if($("#businessPng").hasClass('businessPngchange2')){
            businesssort="define1|ASC";
        }else {
            businesssort = "";
        }
        if(fMemosort !=""){
            sort =fMemosort+","+ sort+"2";
        }
        if(businesssort !=""){
            sort =businesssort+","+sort+"2";
        }
        return sort;
    }
