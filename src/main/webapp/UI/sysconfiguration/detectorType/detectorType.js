...
;(function ( window,$) {
    ...
    function _initEven() {
        ...
        $("#update").bind("click",function () {
            if($("#table_content tr").hasClass("row_isChecked")){
                ...
                _global.top.alertTip("请先选择要修改的探头型号数据！",2000,null);
            }
        });
        //删除
        $("#close").bind("click",function () {
            if($("#table_content tr").hasClass("row_isChecked")){
                if(_global.snModeldata.snModelId == "-1"){
                    _global.top.alertTip("默认设备型号，无法删除！",2000,null);
                }else {
                    _global.top.comfireFloat("是否确认删除探头型号" + _global.snModeldata.snModelId + "?", deletedata, null);
                }
            }
            else{
                _global.top.alertTip("请先选择要删除探头型号！",2000,null);
            }
        });
    }
    //请求
    ...
    function addTableRow(jsonData) {
        ...
        $div_snKind.addClass("eventNum_title6").text(jsonData.snTypeName).attr("title", jsonData.snTypeName);
