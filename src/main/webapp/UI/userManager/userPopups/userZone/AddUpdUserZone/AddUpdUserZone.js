...
(function(window, $) {
	...
	function _initLayout() {
		...
		if (parent.getPopupsRowJson
				&& typeof (parent.getPopupsRowJson) == 'function') {
	...
	function _initEvent() {
		...
		/*
		 * $("#confirmButton").bind('click',function () { _submitDevice(); });
		 */
	...
	function getRoleZoneInfo() {
		...
		post_async(params, _config.ajaxUrl.addRoleZoneUrl,
				_callback_submitContact);
	}
	...
	function _getRoleZoneParams() {
		...
		params.roleZonePojo.devZoneId = $("#devZoneId_input option:selected")
				.val();
	...
	function getDevIdDropdown() {
		...
		post_async({
			"ownerId" : ownerId
		}, _config.ajaxUrl.getDevIdUrl, devIdDrop_callback);
	}
	...
	function getDevZoneId(devId, strChange) {
		post_async({
			"devId" : devId
		}, _config.ajaxUrl.getDevZoneUrl, devZone_callback, strChange);
	}
	function devZone_callback(data, strChange) {
		...
		if (popusName == 'editUserZone') {
			...
			if (strChange != 'change') {
				...
				$("#devId_input option[value='" + editjson.devId + "']").attr(
						"selected", true);
				$("#devZoneId_input option[value='" + editjson.devZoneId + "']")
						.attr("selected", true);
			}
	...
	function _getDevZoneInfo() {
		...
		post_async(params, _config.ajaxUrl.getDevZoneInfoUrl,
				_callback_getDevZoneInfo);
	}
