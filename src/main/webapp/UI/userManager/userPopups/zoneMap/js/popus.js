    ...
    $.extend($.fn, {
        ...
        alert: function(options) {
            ...
            function initialize() {
                ...
                //用户防区编号select
                var roleZoneName = $('<select id="roleZoneName"></select>');
                roleZoneName.css({
                    "width":"176px",
                    "height":"22px",
                    "margin-top":"8px",
                ...
                firstRightDiv.append(roleZoneName);

                var jsonSelect=settings.jsonSelect;
                if(jsonSelect.length>0) {
                    roleZoneName.append(getOption());
                }
                ...
                if(settings.selected!=null) {
                    roleZoneName.val(settings.selected);
                    roleZoneName.attr("disabled","false");
                }
                selected();
                ...
                $("#roleZoneName").bind('change', function () {
                    selected();
                });
            ...
            function selected() {
                var roleZoneNamesel=$("#roleZoneName").val();

                if(roleZoneNamesel==null&&settings.selected!=null)
                {
                    roleZoneNamesel=settings.selected;
                }
                noChang(roleZoneNamesel)
            }
            //取消按钮事件
            ...
            function confirmBtnClickHandler() {
                var roleZoneNamesel=$("#roleZoneName").val();
                var devZoneId = $("#roleZoneName").data('devZoneId');
                if(roleZoneNamesel.length>0) {
                    $("#selectMainDiv").remove();
                    $("#selectBottomDiv").remove();
                    ...
                    for(var i=0;i<jsonArr.length;i++){
                        if(jsonArr[i].id==settings.draggableId){
                            jsonArr[i].roleZoneName=roleZoneNamesel;
                            ...
                            $("#"+settings.draggableId+"Number").html(roleZoneNamesel).css({
            ...
            function noChang(roleZoneNamesel) {
                var jsonSelect=settings.jsonSelect;
                //设置选择值
                for(var si=0;si<jsonSelect.length;si++) {
                    if(roleZoneNamesel==jsonSelect[si].roleZoneName){
                        var snTypeName=jsonSelect[si].snTypeName==""?jsonSelect[si].snType:jsonSelect[si].snTypeName;
                        setSelectec(jsonSelect[si].atPos,snTypeName);
                        $("#roleZoneName").data('devZoneId',jsonSelect[si].devZoneId);
                    }
                }
            }
            function getOption() {
                ...
                for(var si=0;si<jsonSelect.length;si++){
                    ...
                    for(var i=0;i<jsonArr.length;i++){
                        if(jsonArr[i].roleZoneName==jsonSelect[si].roleZoneName){
                            isContent=true;
                            option+="<option value='"+jsonSelect[si].roleZoneName+"' disabled='true'>"+jsonSelect[si].roleZoneName+"</option>";
                        }
                    }
                    if(!isContent){
                        if(!isSelected){
                            isSelected=true;
                            option+="<option value='"+jsonSelect[si].roleZoneName+"' selected>"+jsonSelect[si].roleZoneName+"</option>";
                        }
                        else{
                            option+="<option value='"+jsonSelect[si].roleZoneName+"'>"+jsonSelect[si].roleZoneName+"</option>";
                        }
