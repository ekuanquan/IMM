...
(function(window, $, undefined) {
	...
	function showZoonData(row_json) {
		...
		var roleZoneName = row_json.roleZoneName;
		if(OLits.length > 0) {
			for(var i = 0; i < OLits.length; i++) {
				if(OLits[i] == roleZoneName) {
					break;
				}
				if(i == OLits.length - 1) {
					var $row_row = $('<div></div>');
					...
					$row_Checked.addClass('row_noChecked').attr('id', roleZoneName).appendTo($row_row);
					$span.text(roleZoneName).appendTo($row_row);
	...
	function add_addZoonIdList(roleZoneName) {
		...
		if(addData > 0) {
			for(var i = 0; i < addData.length; i++) {
				var oJsonroleZoneName = addData[i];
				if(roleZoneName == oJsonroleZoneName) {
					break;
				}
				if(i == addData.length - 1) {
					addData.push(roleZoneName);
				}
			}
		} else {
			addData.push(roleZoneName);
	...
	function del_addZoonIdList(roleZoneName) {
		...
		for(var j = 0; j < addData.length; j++) {
			var oJsonroleZoneName = addData[j];
			if(roleZoneName == oJsonroleZoneName) {
				index = j;
				break;
			} else {
