...
(function() {
    $.extend($.fn, {
        ...
        redraw:function(options,isCenter,pos){
            ...
            for(var i=0;i<len;i++){
                var info = data[i];
                var x = $elem.width() * info.x /*+ $elem.offset().left*/;
                var y = $elem.height() * info.y /*+ $elem.offset().top*/;
                ...
                for(var i=0;i<settingsList.dataList.length;i++){
                    if((settingsList.dataList[i].x==null||settingsList.dataList[i].x==0||settingsList.dataList[i].x=="")&&
                        ...
                        if(settingsList.mapId == mapId){
                            ...
                            var x = wrapper.width() * settingsList.dataList[i].x ;//+ wrapper.offset().left - kuangkuang.offset().left;
                            var y = wrapper.height() * settingsList.dataList[i].y ;//+ wrapper.offset().top - kuangkuang.offset().top;
